...
class TDTtranslator {
     ...
     */
    #fetchAllData() {
        const loadSchemaData = this.#fetchDirectoryData('./schemas/')
    ...
    #fetchDirectoryData(baseDir) {
        if (typeof process !== 'undefined' && process.versions && process.versions.node && typeof require === 'function') {
            try {
                ...
                let resolvedDir = path.resolve(process.cwd(), baseDir);
                if (!fs.existsSync(resolvedDir) && typeof __dirname !== 'undefined') {
                    resolvedDir = path.resolve(__dirname, baseDir);
                }
    ...
    /**
     * Fetches and parses standard GS1 Company Prefix format maps to dynamically index allocation profiles.
     * * @private
     * @param {string} url - URL such as 'gcpprefixformatlist.json'.
     * @returns {Promise<void>} Resolves when internal cache tables are loaded completely.
     */
    #fetchPrefixLengthData(url) {
        if (typeof process !== 'undefined' && process.versions && process.versions.node && typeof require === 'function') {
            try {
                ...
                if (!fs.existsSync(filePath) && typeof __dirname !== 'undefined') {
                ...
                if (fs.existsSync(filePath)) {
                    ...
                    console.debug("Optional prefix length table not found on disk; proceeding with manual or default GCP length.");
                    return Promise.resolve();
                }
            } catch (err) {
                // fall through to fetch
        ...
        return fetch(resolveUrl(url))
            ...
            .then(data => {
                // Map incoming array entries into local lookup dictionary properties
                for (let index in data.GCPPrefixFormatList.entry) {
                    let entry = data.GCPPrefixFormatList.entry[index];
                    this.gcpLengths[entry.prefix] = entry.gcpLength;
                }
    ...
    /**
     * Interrogates numeric character sequences systematically to isolate matching 
     * allocated GS1 Company Prefix length values via decreasing length evaluation.
     * * @private
     * @param {string} string - Base barcode numeric data segment (e.g., derived GTIN array).
     * @returns {number} The appropriate length matching known standards, or -1 if unrecognized.
     */
    #lookupPrefixLength(string) {
        let prefix = string.substr(0, 11); // Standard maximum evaluated scope limits
        while (prefix.length > 0) {
            if (prefix in this.gcpLengths) return this.gcpLengths[prefix];
            prefix = prefix.slice(0, -1); // Progressively trim rightmost digit to match shorter prefix ranges
        }
        return -1;
    }
    ...
    /**
     * Encodes individual characters into a dense bit string by stripping non-significant 
     * leading bits based on an explicit bit-width rule context.
     * * @static
     * @param {string} inputCharacterString - Source string text context data.
     * @param {number} bitsPerChr - Density metric instruction flag parameter (Valid rules: 5, 6, 7, 8).
     * @throws {Error} On passing width instructions falling outside standardized specification limits.
     * @returns {string} Binary bit stream encoding of input character string.
     */
    static toBinaryUsingTruncatedASCII(inputCharacterString, bitsPerChr) {
        ...
        for (let i = 0; i < inputCharacterString.length; i++) {
            ...
            // Slice off high bits to match targeted compaction boundaries
    ...
    /**
     * Packs numeric string records directly into a fixed-width binary BigInt bit sequence.
     * compliant with TDS numeric value storage limits.
     * * @static
     * @param {string} inputCharacterString - Numeric plaintext value structure context string.
     * @param {Object} options - Active format configuration specifications profile mapping parameters.
     * @param {number} options.fixLenBits - Explicit allocation maximum bit boundary sizing limits.
     * @throws {Error} If character contents fail numeric parsing, or if value limits exceed binary bit capacities.
     * @returns {string} Sized binary representation text mapping details out cleanly.
     */
    static toBinaryUsingFixedBitLengthInteger(inputCharacterString, options) {
        ...
        // Utilizing native JS BigInt to securely process very large tracking numbers without precision loss
    ...
    /**
     * Packs specialized date fields for medical or critical product configurations 
     * utilizing prioritized mapping arrays inside DSGTIN+ tracking rules.
     * * @static
     * @param {string} inputAIkey - Target verification application identifier key string code.
     * @param {string} inputYYMMDD - Date string tracking values matching format constraints.
     * @throws {Error} If application identifiers or date formats violate standard schema constraints.
     * @returns {string} Condensed tracking code bit string.
     */
    ...
    /**
     * Converts raw numeric characters directly into chunks of 4-bit BCD (Binary Coded Decimal) sequences.
     * * @static
     * @param {string} inputCharacterString - Numeric target information payload text.
     * @returns {string} Consolidated binary stream string.
     */
    ...
    /**
     * Encodes variable-length alphanumeric structures into binary coded decimal (BCD) streams, appending 
     * a specific delimiter code (`1110` or `1111`) to mark data transition boundaries.
     * * @static
     * @param {string} inputCharacterString - Text array components tracking information.
     * @param {Object} options - Global configuration specification reference object map.
     * @returns {string} Compiled tracking bitstream context values.
     */
    static toBinaryUsingDelimitedNumeric(inputCharacterString, options) {
        let outputBinaryString = "";
        let t = 0;
        // Process primary numeric run length loops
        for (t = 0; t < inputCharacterString.length; t++) {
            const value = parseInt(inputCharacterString.charAt(t));
            if (isNaN(value)) {
                break; // Break away immediately if an alphanumeric shift boundary is discovered
            } else {
                outputBinaryString += TDTtranslator.prePad(value.toString(2), "0", 4);
            }
        }
        // Append alphanumeric fallback data if a non-numeric character broke the loop early
        if (t < inputCharacterString.length) {
            outputBinaryString += "1110"; // Alphanumeric shift flag instruction
            outputBinaryString += TDTtranslator.toBinaryUsingVariableLengthAlphanumeric(inputCharacterString.substr(t), options);
        } else {
            outputBinaryString += "1111"; // Standard data block termination instruction flag
        }
        return outputBinaryString;
    }
    ...
    /**
     * Packs unstructured numeric sequences into variable-width binary strings 
     * based on their mathematical log2 scale ceiling footprint.
     * * @static
     * @param {string} inputCharacterString - Raw numerical text block properties.
     * @throws {Error} If properties are undefined or fail standard expression tests.
     * @returns {string} Calculated bit representation properties text string.
     */
    static toBinaryUsingBigInteger(inputCharacterString) {
        ...
        // Calculate bit capacity footprint limits dynamically matching standard base-10 length targets
    ...
    /**
     * Converts uppercase hexadecimal tracking string inputs into raw binary data.
     * * @static
     * @param {string} inputCharacterString - Hex format alphanumeric text input values.
     * @throws {Error} If tracking tokens contain lowercase syntax deviations or incorrect characters.
     * @returns {string} Compiled bit details.
     */
    ...
    /**
     * Converts lowercase hexadecimal tracking string inputs into raw binary data.
     * * @static
     * @param {string} inputCharacterString - Hex format alphanumeric text input values.
     * @throws {Error} If tracking tokens contain uppercase syntax deviations or incorrect characters.
     * @returns {string} Compiled bit details.
     */
    ...
    /**
     * Decodes continuous binary streams into file-safe URL-safe base64 text segments.
     * * @static
     * @param {string} inputBinaryString - Source bits array collection.
     * @throws {Error} If stream sizes are not clean multiples of 6 bits.
     * @returns {string} Restored base64 character string.
     */
    ...
    /**
     * Packs standard URN Code 40 text into high-density 16-bit binary block components.
     * * @static
     * @param {string} inputCharacterString - Valid limited alphabet character string payload context.
     * @throws {Error} If tracking text contains illegal symbols or breaks vocabulary boundaries.
     * @returns {string} Sized binary sequence properties text.
     */
    static toBinaryUsingURNcode40(inputCharacterString) {
        let outputBinaryString = "";
        if (TDTtranslator.regexURNcode40.test(inputCharacterString)) {
            // Apply space character padding to align inputs cleanly into clusters of three
            ...
            for (let t = 0; t < inputCharacterString.length / 3; t++) {
                const i1 = TDTtranslator.alphabetURNcode40.indexOf(inputCharacterString.charAt(3 * t));
                const i2 = TDTtranslator.alphabetURNcode40.indexOf(inputCharacterString.charAt(3 * t + 1));
                const i3 = TDTtranslator.alphabetURNcode40.indexOf(inputCharacterString.charAt(3 * t + 2));
                // Base-40 formula transformation map steps
    ...
    /**
     * Encodes standard characters into 7-bit ASCII text bitstream outputs.
     * * @static
     * @param {string} inputCharacterString - Source plain text tracking entry.
     * @returns {string} Truncated bit data structures text.
     */
    ...
    /**
     * Evaluates text properties against all available codecs to automatically select 
     * and apply the most bit-efficient compaction strategy.
     * * @static
     * @param {string} inputCharacterString - Target string parameters tracking details.
     * @param {Object} options - Config options profiling properties context parameters layout definitions.
     * @param {number} options.lenIndBits - Total number of bits allocated to represent the string length indicator.
     * @throws {Error} If no compatible encoder can map the source text characters securely.
     * @returns {string} Packed bit block consisting of an indicator prefix, length block, and payload.
     */
    static toBinaryUsingVariableLengthAlphanumeric(inputCharacterString, options) {
        let candidates = [];
        const length = inputCharacterString.length;

        // Test each encoder option to calculate its required bit footprint
        ...
        // Sort candidates to select the most efficient encoding option
    ...
    /**
     * Processes a single-bit boolean-style tracking flag string indicator into binary.
     * * @static
     * @param {string} inputCharacterString - Target single bit character ('0' or '1').
     * @throws {Error} If input values break valid single bit bounds.
     * @returns {string} Verified single character binary text.
     */
    ...
    /**
     * Compacts a six-digit date string (`YYMMDD`) into a fixed-width 16-bit binary payload block.
     * Uses 7 bits for the year, 4 bits for the month, and 5 bits for the day.
     * * @static
     * @param {string} inputYYMMDD - Six-character numerical calendar date tracking string.
     * @throws {Error} If date values violate valid calendar structure boundaries.
     * @returns {string} 16-bit binary string block representing calendar data coordinates.
     */
    ...
    /**
     * Compacts a ten-digit timestamp string (`YYMMDDhhmm`) into a fixed-width 27-bit binary payload block.
     * Allocates: 7 bits (YY), 4 bits (MM), 5 bits (DD), 5 bits (hh), and 6 bits (mm).
     * * @static
     * @param {string} inputYYMMDDhhmm - Date and time character sequence values.
     * @throws {Error} If layout paths or value fields violate regular expression checks.
     * @returns {string} Fully structured 27-bit data tracking code.
     */
    ...
    /**
     * Packs a singular date (`YYMMDD`) or a date range (`YYMMDDYYMMDD`) into a binary payload block.
     * Prepends a single tracking flag bit (`0` for singular date, `1` for date range) to establish layout rules.
     * * @static
     * @param {string} inputYYMMDDorYYMMDDYYMMDD - Date or range numeric text components.
     * @throws {Error} On passing undefined parameters or text failing structural validation.
     * @returns {string} Variable-width binary bit block sequence mapping data properties.
     */
    static toBinaryUsingDateOrDateRange(inputYYMMDDorYYMMDDYYMMDD) {
        ...
        // Single Date Mode (allocates 1 + 16 = 17 bits total)
        if (inputYYMMDDorYYMMDDYYMMDD.length === 6) {
            ...
            // Date Range Mode (allocates 1 + 16 + 16 = 33 bits total)
    ...
    /**
     * Compacts variable-precision date strings (`YYMMDD[hh][mm][ss]`) into binary fields.
     * Uses a leading 2-bit indicator to define the precision level and expected layout width.
     * * @static
     * @param {string} inputYYMMDDhh_mmss - Alphanumeric source string containing timestamp values.
     * @throws {Error} If fields contain invalid alphanumeric data patterns or layout configurations.
     * @returns {string} Packed bit stream mapping parameters cleanly.
     */
    static toBinaryUsingVariablePrecisionDateTime(inputYYMMDDhh_mmss) {
        ...
        // Establish the precision prefix bits based on input character length metrics
        if (length === 8) {
            binaryString = "00"; // Year, Month, Day, Hour
        } else if (length === 10) {
            binaryString = "01"; // Year, Month, Day, Hour, Minute
        } else if (length === 12) {
            binaryString = "10"; // Year, Month, Day, Hour, Minute, Second
        } else if (length === 6) {
            binaryString = "11"; // Year, Month, Day only
        }
        ...
        // Conditionally append sub-components according to the precision limits
        if (length > 6) {
    ...
    /**
     * Packs a standard two-letter ISO 3166 country code into a 12-bit binary structure 
     * by leveraging the URL-safe base64 compaction codec.
     * * @static
     * @param {string} inputCountryCode - Two-letter uppercase ISO 3166 country code text reference.
     * @throws {Error} If fields are undefined or break standard character regex syntax.
     * @returns {string} Fully packaged binary bit stream mapping data properties.
     */
    ...
    /**
     * Encodes variable-length numerical fields into binary layout properties 
     * by prepending a explicit binary length indicator block.
     * * @static
     * @param {string} inputCharacterString - Numeric plaintext field code parameters.
     * @param {Object} options - Configuration reference object mapping tracking metrics rules.
     * @param {number} options.lenIndBits - Allocated width in bits for the prepended length block.
     * @returns {string} Combined binary payload data tracking string.
     */
    ...
    /**
     * Packs an optional algebraic minus sign into a single bit flag (`1` for `-`, `0` for empty string).
     * * @static
     * @param {string} inputCharacterString - Text character target input data string.
     * @throws {Error} If fields contain characters other than a minus symbol or an empty string.
     * @returns {string} Single character bit payload sequence context.
     */
    static toBinaryUsingOptionalMinus(inputCharacterString) {
        if (!TDTtranslator.regexOriginalMinus || !TDTtranslator.regexOptionalMinus.test(inputCharacterString)) {
    ...
    /**
     * Static property getter mapping standard TDS specification section chapters 
    ...
    static get tds2encodingMethods() {
        return {
            ...
            "14.5.3": {}, // Reserved section parameter placeholder matching external spec structures
    ...
    /**
     * Generic structural routing interface that resolves the correct compaction codec 
     * for a given text input based on configuration instructions parsed from Table F.
     * * @static
     * @param {string} inputCharacterString - Plain text context segment needing encryption.
     * @param {Object} options - Parameter properties defining size rules and structural details.
     * @param {string} options.section - The target TDS section identifier (e.g., '14.5.2').
     * @param {number} [options.maxChars] - Maximum character limits constraint rule parameters.
     * @param {number} [options.fixLenChrs] - Fixed character constraint limits tracking specifications.
     * @throws {Error} On passing data content lengths violating configuration rules or missing codec matching markers.
     * @returns {Object} Tracking results data layout structure mapping {"binary": string, "encodingMethod": Object}.
     */
    static toBinaryUsingTableF(inputCharacterString, options) {
        const rv = {};
        // Assess if the input string is an uncompacted raw binary bitstream profile
        ...
        if (!TDTtranslator.tds2encodingMethods[options.section]) {
            ...
            // Route execution down onto isolated destination encoder components
    ...
    /**
     * Scans parsed JSON objects to isolate peripheral data elements (+AIDC / Plus Data) 
     * that fall outside the core standard EPC identification sequence keys.
     * * @static
     * @param {string} inputJSONString - Text containing the source serialized JSON map stream.
    ...
    /**
     * Decompresses custom compressed Web URL signatures, converts them back to binary payloads, 
     * auto-detects their properties, and returns fully expanded GS1 Digital Link text representations.
     * * @param {string} inputString - Compressed URL sequence string (e.g., housing base64 or hex suffix keys).
     * @param {string} overridingURIstem - Optional custom domain stem parameter override.
     * @returns {string} Fully expanded, standardized GS1 Digital Link text URL structure.
     */
    handleCompressedDL(inputString, overridingURIstem) {
        ...
        // Decompress URL patterns that match compressed base64 or hex syntax rules
        if ((regexFileSafeURISafeBase64compressedDL.test(inputString)) || (regexHexcompressedDL.test(inputString)) ) {
            ...
            // Run auto-detection on the unpacked binary string payload
            let detected = this.autodetect(inputString);
            if (detected && detected.length > 0) {
                let options = { "filter": 0, "uriStem": uriStem, "gs1companyprefixlength": -1 };
                let translated = null;
                // Execute master translator conversion targeting GS1 Digital Link syntax structures
            ...
            return inputString; // Return input unchanged if it was already an uncompressed string structure
        }
    }
    ...
    /**
     * Diagnostic data helper exposing a list of all currently supported 
     * EPC schema definitions loaded into memory.
     * * @returns {Array<string>} List housing active schema profile identifiers.
     */
    schemes() {
        if (this.tdtData && this.tdtData.scheme) {
    ...
    /**
     * Utility method to convert raw binary bitstreams into uppercase hexadecimal text representation blocks.
     * * @param {string} inputBinaryString - Stream sequence text consisting strictly of 0 and 1 characters.
     * @throws {Error} If character sequences contain data violating binary rules.
     * @returns {string} Fully formatted uppercase hexadecimal string.
     */
    bin2hex(inputBinaryString) {
        ...
        for (let t = 0; t < inputBinaryString.length; t += 4) {
            ...
            binary = TDTtranslator.postPad(binary, "0", 4); // Handle unaligned trailing bits cleanly
...
// CommonJS module export for headless Node.js batch processing
if (typeof module !== 'undefined' && module.exports) {
    module.exports = TDTtranslator;
}